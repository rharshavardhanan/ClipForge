...
export async function runRankRot(topic: string, opts: RankRotOpts): Promise<string> {
  ...
  const aiDropped = kept.filter((_, i) => virality.aiFlags[i]).length;
  if (aiDropped > 0) logger.info(`${aiDropped} clip(s) flagged AI-generated by Gemini — excluded (real footage only)`);

  const scored: ScoredClip[] = kept
    .filter((_, i) => !virality.aiFlags[i])
    .map((k) => {
      const i = kept.indexOf(k);
      const layers: LayerScores = {
        visual: k.visualN, audio: k.audioN, reaction: k.reaction,
        virality: virality.scores[i], novelty: novelty[i],
      };
      return {
  ...
  const items: RankRotRenderItem[] = picks.map((c, i) => ({
    ...
    // Slow-mo replay re-shows the fused PEAK of the moment, not its opening seconds.
    peakSec: analyzed.find((a) => a.momentFile === c.momentFile)?.peakRelSec ?? 0,
  }));
  logger.info(`Rank lengths: ${items.map((it) => `#${it.rank}=${it.durationSec.toFixed(1)}s`).join(' ')} (adaptive, arcs preserved)`);
  ...
  await writeFile(join(exportsDir, 'rankrot_manifest.json'), JSON.stringify({
    ...
    picks: picks.map((c, i) => ({
      ...
      views: c.candidate.viewCount ?? null,
